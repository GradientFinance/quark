...
import "@uma/core/contracts/data-verification-mechanism/interfaces/OracleInterface.sol";
...
contract LongShortPair is Testable, Lockable {
    ...
    struct ConstructorParams {
        ...
        uint256 feeSize; // Percentage of collateralPerPair that is charged as a fee (multiplied by 1e18)
        IERC721 feeToken; // ERC721 token for fees
        uint256 feeTokenId; // token id of the recipient ERC721 token
    }
    ...
    // Fees
    uint256 public feeSize; // Percentage of collateralPerPair that is charged as a fee (multiplied by 1e18)
    IERC721 public feeToken; // ERC721 token for fees
    uint256 public feeTokenId; // token id of the recipient ERC721 token
    ...
    event FeesClaimed(uint256 indexed amountClaimed, address indexed recipient);
    ...
    modifier onlyFeeTokenOwner() {
        require(msg.sender == feeToken.ownerOf(feeTokenId), "Caller must be fee token owner");
        _;
    }
    ...
    constructor(ConstructorParams memory params) {
        ...
        require(params.feeSize < 1e18, "The fee percentage is too large");
        require(address(params.feeToken) != address(0), "Invalid finder");
        ...
        feeSize = params.feeSize;
        feeToken = params.feeToken;
        feeTokenId = params.feeTokenId;
     ...
     */
    function create(uint256 tokensToCreate, bytes calldata ancillaryData) public preExpiration() nonReentrant() returns (uint256 collateralUsed) {
        ...
        uint256 tokenId = uint256(keccak256(abi.encodePacked(ancillaryData))) * 2;

        require(_mintBatch(msg.sender, [tokenId,tokenId+1], [tokensToCreate,tokensToCreate], "0x0"));
    ...
    function redeem(uint256 tokensToRedeem) public nonReentrant() returns (uint256 collateralReturned) {
        ...
        collateralReturned = FixedPoint
                                .Unsigned(tokensToRedeem)
                                .mul(FixedPoint.Unsigned(collateralPerPair))
                                .mul(FixedPoint.fromUnscaledUint(1).sub(FixedPoint.Unsigned(feeSize)))
                                .rawValue;
    ...
    function claimFees(uint256 claimAmount, address recipient) external nonReentrant() onlyFeeTokenOwner() {
        require(claimAmount <= getClaimableFeesAmount(), "Claim amount too large");

        collateralToken.safeTransfer(recipient, claimAmount);

        emit FeesClaimed(claimAmount, recipient);
    }
    ...
    function getClaimableFeesAmount() public view nonReentrantView() returns (uint256 claimableFees) {
        require(longToken.totalSupply() == shortToken.totalSupply(), "There's not the same amount of long and short tokens");

        uint256 collateralAmount = FixedPoint.Unsigned(longToken.totalSupply()).mul(FixedPoint.Unsigned(collateralPerPair)).rawValue;

        claimableFees = FixedPoint.Unsigned(collateralToken.balanceOf(address(this))).sub(FixedPoint.Unsigned(collateralAmount)).rawValue;
    }
